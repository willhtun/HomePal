...
public class Internet extends AppCompatActivity {
    ...
    public void internet_personX_pay(int personNum, ImageView internet_pX_button, int internet_buttontextID) { //R.id.internet_person1_text
        mDatabaseHelper.addData_toHistory(type, mCalendarHelper.getCycleMonthYear(type), personNum, personNum, price);
        internet_pX_button.setImageResource(R.drawable.generic_personpaidbutton);
        ((TextView) findViewById(internet_buttontextID)).setTextColor(Color.WHITE);
        loadHistory();
    }

    public void loadHousemates() {
        int text[] = {R.id.internet_person1_text, R.id.internet_person2_text, R.id.internet_person3_text, R.id.internet_person4_text, R.id.internet_person5_text};
        for (int i = 1; i < 6; i++) {
    ...
    public void loadButtons() {
        if (mDatabaseHelper.isDataExists_fromHistory(type, mCalendarHelper.getCycleMonthYear(type),0)) {
            ...
            ((TextView) findViewById(R.id.internet_payBtn_text)).setTextColor(Color.parseColor("#000000"));
        }

        if (preferences.getBoolean("settings_billSharingOn_internet", false)) {
            final int text[] = {R.id.internet_person1_text, R.id.internet_person2_text, R.id.internet_person3_text, R.id.internet_person4_text, R.id.internet_person5_text};
            final ImageView button [] = {internet_p1_btn, internet_p2_btn, internet_p3_btn, internet_p4_btn, internet_p5_btn};
            ...
            for (int i = 1;i < 6; i++) {
                if (!preferences.getBoolean("housemate" + i + "_On", false)) {
                    ...
                    button[i-1].setOnLongClickListener(new View.OnLongClickListener() {
                        @Override
                        public boolean onLongClick(View v) {
                            internet_personX_pay(person_nump_temp, button[person_nump_temp-1], text[person_nump_temp-1]);
                            return true;
                        }
                    });
                }
    ...
    public void loadHistory() {
        LinearLayout vertical_ll = findViewById(R.id.internet_history);
