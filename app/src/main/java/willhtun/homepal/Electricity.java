...
public class Electricity extends AppCompatActivity {
    ...
    public void electricity_personX_pay(int personNum, ImageView electricity_pX_button, int electricity_buttontextID) { //R.id.electricity_person1_text
        mDatabaseHelper.addData_toHistory(type, mCalendarHelper.getCycleMonthYear(type), personNum, personNum, price);
        electricity_pX_button.setImageResource(R.drawable.generic_personpaidbutton);
        ((TextView) findViewById(electricity_buttontextID)).setTextColor(Color.WHITE);
        loadHistory();
    }

    public void loadHousemates() {
        int text[] = {R.id.electricity_person1_text, R.id.electricity_person2_text, R.id.electricity_person3_text, R.id.electricity_person4_text, R.id.electricity_person5_text};
        for (int i = 1; i < 6; i++) {
    ...
    public void loadButtons() {
        if (mDatabaseHelper.isDataExists_fromHistory(type, mCalendarHelper.getCycleMonthYear(type),0)) {
            ...
            ((TextView) findViewById(R.id.electricity_payBtn_text)).setTextColor(Color.parseColor("#000000"));
        }

        if (preferences.getBoolean("settings_billSharingOn_electricity", false)) {
            final int text[] = {R.id.electricity_person1_text, R.id.electricity_person2_text, R.id.electricity_person3_text, R.id.electricity_person4_text, R.id.electricity_person5_text};
            final ImageView button [] = {electricity_p1_btn, electricity_p2_btn, electricity_p3_btn, electricity_p4_btn, electricity_p5_btn};
            ...
            for (int i = 1;i < 6; i++) {
                if (!preferences.getBoolean("housemate" + i + "_On", false)) {
                    ...
                    button[i-1].setOnLongClickListener(new View.OnLongClickListener() {
                        @Override
                        public boolean onLongClick(View v) {
                            electricity_personX_pay(person_nump_temp, button[person_nump_temp-1], text[person_nump_temp-1]);
                            return true;
                        }
                    });
                }
    ...
    public void loadHistory() {
        LinearLayout vertical_ll = findViewById(R.id.electricity_history);
