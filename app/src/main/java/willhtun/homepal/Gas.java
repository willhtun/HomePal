...
public class Gas extends AppCompatActivity {
    ...
    public void gas_personX_pay(int personNum, ImageView gas_pX_button, int gas_buttontextID) { //R.id.gas_person1_text
        mDatabaseHelper.addData_toHistory(type, mCalendarHelper.getCycleMonthYear(type), personNum, personNum, price);
        gas_pX_button.setImageResource(R.drawable.generic_personpaidbutton);
        ((TextView) findViewById(gas_buttontextID)).setTextColor(Color.WHITE);
        loadHistory();
    }

    public void loadHousemates() {
        int text[] = {R.id.gas_person1_text, R.id.gas_person2_text, R.id.gas_person3_text, R.id.gas_person4_text, R.id.gas_person5_text};
        for (int i = 1; i < 6; i++) {
    ...
    public void loadButtons() {
        if (mDatabaseHelper.isDataExists_fromHistory(type, mCalendarHelper.getCycleMonthYear(type),0)) {
            ...
            ((TextView) findViewById(R.id.gas_payBtn_text)).setTextColor(Color.parseColor("#000000"));
        }

        if (preferences.getBoolean("settings_billSharingOn_gas", false)) {
            final int text[] = {R.id.gas_person1_text, R.id.gas_person2_text, R.id.gas_person3_text, R.id.gas_person4_text, R.id.gas_person5_text};
            final ImageView button [] = {gas_p1_btn, gas_p2_btn, gas_p3_btn, gas_p4_btn, gas_p5_btn};
            ...
            for (int i = 1;i < 6; i++) {
                if (!preferences.getBoolean("housemate" + i + "_On", false)) {
                    ...
                    button[i-1].setOnLongClickListener(new View.OnLongClickListener() {
                        @Override
                        public boolean onLongClick(View v) {
                            gas_personX_pay(person_nump_temp, button[person_nump_temp-1], text[person_nump_temp-1]);
                            return true;
                        }
                    });
                }
    ...
    public void loadHistory() {
        LinearLayout vertical_ll = findViewById(R.id.gas_history);
