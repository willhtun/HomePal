...
public class Car extends AppCompatActivity {
    ...
    public void car_personX_pay(int personNum, ImageView car_pX_button, int car_buttontextID) { //R.id.car_person1_text
        mDatabaseHelper.addData_toHistory(type, mCalendarHelper.getCycleMonthYear(type), personNum, personNum, price);
        car_pX_button.setImageResource(R.drawable.generic_personpaidbutton);
        ((TextView) findViewById(car_buttontextID)).setTextColor(Color.BLACK);
        loadHistory();
    }

    public void loadHousemates() {
        int text[] = {R.id.car_person1_text, R.id.car_person2_text, R.id.car_person3_text, R.id.car_person4_text, R.id.car_person5_text};
        for (int i = 1; i < 6; i++) {
    ...
    public void loadButtons() {
        if (mDatabaseHelper.isDataExists_fromHistory(type, mCalendarHelper.getCycleMonthYear(type),0)) {
            ...
            ((TextView) findViewById(R.id.car_payBtn_text)).setTextColor(Color.parseColor("#AAAAAA"));
        }

        if (preferences.getBoolean("settings_billSharingOn_car", false)) {
            final int text[] = {R.id.car_person1_text, R.id.car_person2_text, R.id.car_person3_text, R.id.car_person4_text, R.id.car_person5_text};
            final ImageView button [] = {car_p1_btn, car_p2_btn, car_p3_btn, car_p4_btn, car_p5_btn};
            ...
            for (int i = 1;i < 6; i++) {
                if (!preferences.getBoolean("housemate" + i + "_On", false)) {
                    ...
                    button[i-1].setOnLongClickListener(new View.OnLongClickListener() {
                        @Override
                        public boolean onLongClick(View v) {
                            car_personX_pay(person_nump_temp, button[person_nump_temp-1], text[person_nump_temp-1]);
                            return true;
                        }
                    });
                }
    ...
    public void loadHistory() {
        LinearLayout vertical_ll = findViewById(R.id.car_history);
