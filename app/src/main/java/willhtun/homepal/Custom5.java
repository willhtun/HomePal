...
public class Custom5 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ((TextView) findViewById(R.id.home_title)).setText(preferences.getString("name_preference_bills_custom5", "Custom 5"));
    ...
    public void custom5_personX_pay(int personNum, ImageView custom5_pX_button, int custom5_buttontextID) { //R.id.custom5_person1_text
        mDatabaseHelper.addData_toHistory(type, mCalendarHelper.getCycleMonthYear(type), personNum, personNum, price);
        custom5_pX_button.setImageResource(R.drawable.generic_personpaidbutton);
        ((TextView) findViewById(custom5_buttontextID)).setTextColor(Color.BLACK);
        loadHistory();
    }

    public void loadHousemates() {
        int text[] = {R.id.custom5_person1_text, R.id.custom5_person2_text, R.id.custom5_person3_text, R.id.custom5_person4_text, R.id.custom5_person5_text};
        for (int i = 1; i < 6; i++) {
    ...
    public void loadButtons() {
        if (mDatabaseHelper.isDataExists_fromHistory(type, mCalendarHelper.getCycleMonthYear(type),0)) {
            ...
            ((TextView) findViewById(R.id.custom5_payBtn_text)).setTextColor(Color.parseColor("#AAAAAA"));
        }

        if (preferences.getBoolean("settings_billSharingOn_custom5", false)) {
            final int text[] = {R.id.custom5_person1_text, R.id.custom5_person2_text, R.id.custom5_person3_text, R.id.custom5_person4_text, R.id.custom5_person5_text};
            final ImageView button [] = {custom5_p1_btn, custom5_p2_btn, custom5_p3_btn, custom5_p4_btn, custom5_p5_btn};
            ...
            for (int i = 1;i < 6; i++) {
                if (!preferences.getBoolean("housemate" + i + "_On", false)) {
                    ...
                    button[i-1].setOnLongClickListener(new View.OnLongClickListener() {
                        @Override
                        public boolean onLongClick(View v) {
                            custom5_personX_pay(person_nump_temp, button[person_nump_temp-1], text[person_nump_temp-1]);
                            return true;
                        }
                    });
                }
    ...
    public void loadHistory() {
        LinearLayout vertical_ll = findViewById(R.id.custom5_history);
